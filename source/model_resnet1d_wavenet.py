...
class WaveNetBlock(nn.Module):
    def __init__(self, dilation_rate, residual_channels, skip_channels, kernel_size):
        ...
        self.dilated_conv = nn.Conv1d(residual_channels, residual_channels, kernel_size, dilation=dilation_rate)
    ...
    def forward(self, x):
        ...
        residual_output = self.residual_conv(dilated_output)
        skip_output = self.skip_conv(dilated_output)
        skip_output = skip_output[:, :, -(self.dilation_rate + 1)::self.dilation_rate]
        residual_output += x[:, :, -residual_output.size(2):]
        return residual_output, skip_output


class WaveNet(nn.Module):
    def __init__(self, num_inputs, num_outputs, num_layers, num_channels, kernel_size):
        ...
        self.initial_conv = nn.Conv1d(num_inputs, num_channels, kernel_size=1)
        self.wavenet_blocks = nn.ModuleList([
            WaveNetBlock(2 ** i, num_channels, num_channels, kernel_size) for i in range(num_layers)
        ])
        self.output_conv1 = nn.Conv1d(num_channels, num_channels, kernel_size=1)
        self.output_conv2 = nn.Conv1d(num_channels, num_outputs, kernel_size=1)

    def forward(self, x):
        x = self.initial_conv(x)
        skip_connections = []
        for block in self.wavenet_blocks:
            residual_output, skip_output = block(x)
            x = x[:, :, -residual_output.size(2):] + residual_output
        ...
        x = x.view(x.size(0), x.size(1), -1)  # Flatten the tensor along the temporal dimension
        x = torch.mean(x, dim=2)  # Global average pooling along the temporal dimension
        return x
