...
class SVRCascade:
    ...
    def test(self, test_feature, true_label=None, true_responses=None):
        ...
        feature_cv=feature_cv.reshape(feature_cv.shape[0],feature_cv.shape[1]*feature_cv.shape[2])
...
def run_test(network, data_loader, device, eval_mode=True):
    model = load_model_from_file("D:\\000_Mora\\FYP\RIDI\\ridi_imu_models\\svr_cascade0308")
    ...
    for bid, (feat, targ, _, _) in enumerate(data_loader):
        ...
        feat_clone=torch.clone(feat)
        feat_clone=torch.transpose(feat_clone,1,2)
        import pdb
        pdb.set_trace()
        pred_2=np.array(model.test(feat_clone.numpy()))
        targets_all.append(targ.detach().numpy())
        preds_all.append(pred)
        preds_2_all.append(pred_2)
        print(np.mean((np.concatenate(targets_all,axis=0) - np.concatenate(preds_all,axis=0)) ** 2, axis=0))
        print(np.mean((np.concatenate(targets_all, axis=0) - np.concatenate(preds_2_all, axis=0)) ** 2, axis=0))
    targets_all = np.concatenate(targets_all, axis=0)
    preds_all = np.concatenate(preds_all, axis=0)
    preds_2_all=np.concatenate(preds_2_all,axis=0)
    losses = np.mean((targets_all - preds_all) ** 2, axis=0)
    losses_2 = np.mean((targets_all - preds_2_all) ** 2, axis=0)
    print(losses)
    print(losses_2)
    return targets_all, preds_all,preds_2_all
...
def test_sequence(args):
    ...
    for data in test_data_list:
        ...
        targets, preds,preds_2 = run_test(network, seq_loader, device, True)
        losses = np.mean((targets - preds) ** 2, axis=0)
        losses_2=np.mean((targets - preds_2) ** 2, axis=0)
        print(losses)
        print(losses_2)
        preds_seq.append(preds_2)
        targets_seq.append(targets)
        losses_seq.append(losses_2)

        pos_pred = recon_traj_with_preds(seq_dataset, preds_2)[:, :2]
