...
def intergreate_acce(imu, init_velocity,device):
    ...
    acceleration = imu[:, 3:, ]
    init_velocity = init_velocity.unsqueeze(1)

    acceleration = acceleration.permute(1, 0, 2)
    acceleration = acceleration.reshape(3, batch_size * n_sequence)
    time_step = 1.0 / n_sequence
    velocity = torch.cumsum(acceleration, dim=1) * time_step + init_velocity

    x_velocity = velocity[0, :]
    y_velocity = velocity[1, :]
    z_velocity = velocity[2, :]

    x_velocity = x_velocity.reshape(batch_size, n_sequence)
    x_velocity = torch.mean(x_velocity, dim=1).reshape(batch_size, 1)
    y_velocity = y_velocity.reshape(batch_size, n_sequence)
    y_velocity = torch.mean(y_velocity, dim=1).reshape(batch_size, 1)
    z_velocity = z_velocity.reshape(batch_size, n_sequence)
    z_velocity = torch.mean(z_velocity, dim=1).reshape(batch_size, 1)
    x_y_z_full = torch.cat((x_velocity, y_velocity, z_velocity), dim=1)
    # x_y_z_mean=torch.cat((x_velocity[-1].unsqueeze(0),y_velocity[-1].unsqueeze(0),z_velocity[-1].unsqueeze(0)),dim=1)
